...
class ComisionesDocumentosController extends Controller
{

	public $content = ['result' => 'error', 'message' => ''];

	public function getAll () {
		$sql="SELECT * from com_comisiones_files";
    ...
    {    
        ...
        $documento = DocumentosComisiones::findFirst($id);
        $path = $_SERVER["DOCUMENT_ROOT"] . '/public/assets/documentos_comisiones/' .$documento->id.'.'.$documento->extension;
	...
	public function uploadDocumento()
    {
        try {
            ...
            if ($this->request->hasFiles()) {
                ...
                $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/public/assets/documentos_comisiones/';
                ...
                foreach ($this->request->getUploadedFiles() as $file) {
                    ...
                    if($sizeA>100||$sizeA===0) {
                        ...
                        if(strtolower($file->getExtension())==='jpg'||strtolower($file->getExtension())==='jpeg'||strtolower($file->getExtension())==='png'||strtolower($file->getExtension())==='pdf' || strtolower($file->getExtension())==='xml' || strtolower($file->getExtension())==='docx') {
                            //var_dump('renombre');
                            $archivo = new DocumentosComisiones();
                            $archivo->setTransaction($tx);
                            $archivo->extension = $file->getExtension();
                            $archivo->comision_id = $request['comision_id'];
    ...
    public function deleteFormato () {
        try {
            ...
            $request = $this->request->getPost();
            if ($request['id']) {
                $documento = DocumentosComisiones::findFirst($request['id']);
                $documento->setTransaction($tx);

                if ($documento->delete()) {
                    $this->content['result'] = 'success';
                    $nombre_fichero = $_SERVER["DOCUMENT_ROOT"] . '/public/assets/documentos_comisiones/'.$documento->id.'.'.$documento->extension;
