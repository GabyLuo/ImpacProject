...
class DocumentosPerfilesController extends Controller
{

	public $content = ['result' => 'error', 'message' => ''];

	public function getAll () {
		$sql="SELECT * from per_documentos";
	...
	public function getByPerfil ($id) {
		$sql="SELECT * from per_documentos where perfil_id = $id";
        ...
        foreach ($documentos as $elemento){
            ...
            $documento->perfil_id = $elemento['perfil_id'];
            $documento->descripcion = $elemento['descripcion'];
    ...
    {    
        ...
        $documento = DocumentosPerfiles::findFirst($id);
        $path = $_SERVER["DOCUMENT_ROOT"] . '/public/assets/documentos_perfiles/' .$documento->id.'.'.$documento->extension;
        ...
        $response->setFileToSend($path, "\"".$documento->descripcion.'.'.$documento->extension."\"");
        $response->send();
        return $response;
    }
	...
	public function uploadDocumento()
    {
        try {
            ...
            if ($this->request->hasFiles()) {
                ...
                $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/public/assets/documentos_perfiles/';
                ...
                foreach ($this->request->getUploadedFiles() as $file) {
                    ...
                    if($sizeA>100||$sizeA===0) {
                        ...
                        if(strtolower($file->getExtension())==='jpg'||strtolower($file->getExtension())==='jpeg'||strtolower($file->getExtension())==='png'||strtolower($file->getExtension())==='pdf' || strtolower($file->getExtension())==='xml' || strtolower($file->getExtension())==='docx') {
                            //var_dump('renombre');
                            $archivo = new DocumentosPerfiles();
                            $archivo->setTransaction($tx);
                            $archivo->extension = $file->getExtension();
                            $archivo->perfil_id = $request['perfil_id'];
    ...
    public function deleteFormato () {
        try {
            ...
            $request = $this->request->getPost();
            if ($request['id']) {
                $documento = DocumentosPerfiles::findFirst($request['id']);
                $documento->setTransaction($tx);

                if ($documento->delete()) {
                    $this->content['result'] = 'success';
                    $nombre_fichero = $_SERVER["DOCUMENT_ROOT"] . '/public/assets/documentos_perfiles/'.$documento->id.'.'.$documento->extension;
