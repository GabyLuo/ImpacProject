...
class TitulosPerfilesController extends Controller
{
    ...
    public function getAll ()
    {
        $sql = "SELECT * 
            FROM per_titulos
            ORDER BY archivo";
        $this->content['titulos'] = $this->db->query($sql)->fetchAll();
    ...
    public function uploadTitulo () {
        try {
            ...
            if ($this->request->hasFiles()) {
                $upload_dir = $_SERVER["DOCUMENT_ROOT"] . '/public/assets/titulos_perfiles/';
                ...
                foreach ($this->request->getUploadedFiles() as $file) {
                    ...
                    if($sizeA>100||$sizeA===0){
                        ...
                        if(strtolower($file->getExtension())==='jpg'||strtolower($file->getExtension())==='jpeg'||strtolower($file->getExtension())==='png'||strtolower($file->getExtension())==='pdf' || strtolower($file->getExtension())==='xml' || strtolower($file->getExtension())==='docx') {
                                $titulos = new Titulos();
                                $titulos->setTransaction($tx);
                                $titulos->perfil_id = intval($request['id']);
                                $titulos->archivo = $request['nombre'];
                                $titulos->extension = $file->getExtension();
                                if ($titulos->create()) {
                                    $nombre_con_id = $titulos->id .'.'. $file->getExtension();
                                    ...
                                    $this->content['error'] = Helpers::getErrors($titulos);
    ...
    {    
        $documento = Titulos::findFirst($id);
        $path = $_SERVER["DOCUMENT_ROOT"] . '/public/assets/titulos_perfiles/' .$documento->id.'.'.$documento->extension;
    ...
    public function deleteFormato() {
        try {
            $tx = $this->transactions->get();
            $request = $this->request->getPost();
            $documento = Titulos::findFirst($request['id']);
                if($documento){
                    $documento->setTransaction($tx);
                    $nombre_fichero = $_SERVER["DOCUMENT_ROOT"] . '/public/assets/titulos_perfiles/'.$documento->id.'.'.$documento->extension;
