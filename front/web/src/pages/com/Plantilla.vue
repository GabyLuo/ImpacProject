      ...
      <div class="q-pa-sm panel-header">
        <div class="row">
          ...
          <div class="col-sm-9 pull-right">
            ...
            <q-btn @click="updateFechasAll()" class="btn_nuevo" icon="add">
                Nuevo Proveedor
              </q-btn>
          </div>
        </div>
          ...
          <div class="col q-pa-md">
            <q-table
                  ...
                  <template slot="body" slot-scope="props">
                    <q-tr :props="props">
                      <q-td key="nombre" :props="props">{{ props.row.apellido_paterno }} {{ props.row.apellido_materno }} {{ props.row.nombre }}</q-td>
          ...
          <div class="row bg-white border-panel">
            <div class="col q-pa-md">
              <div class="row q-col-gutter-xs">
                <div class="col-sm-3">
                  <q-field icon="person" icon-color="dark" :error="$v.form.fields.apellido_paterno.$error" error-label="Escriba el apellido paterno">
                    <q-input upper-case v-model="form.fields.apellido_paterno" type="text" inverted color="dark" float-label="Apellido paterno" maxlength="100"/>
                  </q-field>
                </div>
                <div class="col-sm-3">
                  <q-field icon="person" icon-color="dark" :error="$v.form.fields.apellido_materno.$error" error-label="Escriba el apellido materno">
                    <q-input upper-case v-model="form.fields.apellido_materno" type="text" inverted color="dark" float-label="Apellido materno" maxlength="100"/>
                  </q-field>
                </div>
                <div class="col-sm-3">
            ...
            <div class="col-sm-6">
              <div class="q-pa-sm q-gutter-sm">
                <q-breadcrumbs align="left">
                  <q-breadcrumbs-el label="" icon="home" to="/" />
                  <q-breadcrumbs-el label="Aliados" to="" @click.native="setView('grid')"/>
                  <q-breadcrumbs-el label="Modificar aliado"/>
                </q-breadcrumbs>
              </div>
            </div>
            <div class="col-sm-6 pull-right">
      ...
      <!-- <div class="row" v-if="views.update">
        <div class="col-sm-12">
          <div class="row justify-between">
            <div class="col-sm-9">
              <h5 style="margin: 7px 0; font-weight: bold">MODIFICAR ALIADO - {{ form.fields.nombre }}</h5>
            </div>
            <div class="col-sm-3">
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    next(vm => {
      ...
      for (let i = 0; i < propiedades.length; i++) {
        if (propiedades[i].toUpperCase() === 'admin'.toUpperCase() || propiedades[i].toUpperCase() === 'ROOT'.toUpperCase() || propiedades[i].toUpperCase() === 'CLIENTE'.toUpperCase() || propiedades[i].toUpperCase() === 'LIDER'.toUpperCase() || propiedades[i].toUpperCase() === 'COORDINADOR'.toUpperCase() || propiedades[i].toUpperCase() === 'GERENTE DE OPERACIONES'.toUpperCase() || propiedades[i].toUpperCase() === 'PMO'.toUpperCase() || propiedades[i].toUpperCase() === 'FINANZAS'.toUpperCase() || propiedades[i].toUpperCase() === 'OPERACIÓN'.toUpperCase() || propiedades[i].toUpperCase() === 'LICITACIONES'.toUpperCase() || propiedades[i].toUpperCase() === 'LICITACIONES - SOLICITUDES'.toUpperCase() || propiedades[i].toUpperCase() === 'INVENTARIOS'.toUpperCase() || propiedades[i].toUpperCase() === 'GERENTE'.toUpperCase()) {
          condicion = true
        }
      }
      if (condicion) {
  ...
  methods: {
    ...mapActions({
      ...
      fechasSolicitud: 'fin/solicitudes/update_fechas'
    }),
    async loadAll () {
    ...
    updateFechasAll () {
      this.fechasSolicitud().then(({data}) => {
        if (data.result === 'success') {
          this.$q.notify({
            message: data.message.content, timeout: 3000, type: 'green', textColor: 'white', icon: 'clear', position: 'top-right'
          })
        } else {
          this.$showMessage(data.message.title, data.message.content)
        }
  ...
  validations: {
    form: {
      fields: {
        apellido_paterno: { required, maxLength: maxLength(100) },
        apellido_materno: { required, maxLength: maxLength(100) },
