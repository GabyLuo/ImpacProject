        ...
        <div class="col-sm-6">
          <div class="q-pa-sm q-gutter-sm">
            <q-breadcrumbs align="left">
              ...
              <q-breadcrumbs-el class="page-title" label="Auditoría de projects" to="/" />
            </q-breadcrumbs>
          </div>
        </div>
        <!-- -->
...
export default {
  ...
  data () {
    return {
      ...
      tiposOptions: [{label: 'Projects con cantidad ejercida en actividades no afectables', value: 0}, {label: 'Projects con actividades que tengan cantidad ejercida en negativo', value: 1}],
      views: {
        grid: true,
        create: false,
        update: false
      ...
      form: {
        ...
        columns: [
          ...
          { name: 'utilidad_projects', label: 'Utilidad proyectada antes de comisiones', field: 'utilidad_projects', sortable: true, type: 'string', align: 'right' }, */
  ...
  methods: {
    ...
    async cargarDatosReporte () {
      this.form.loading = true
      this.form.data = []
      await axios.get(`/reportes/getReporteAuditoria/${this.form.fields.estado_id}/${this.form.fields.municipio_id}/${this.form.fields.tipo}`).then(({data}) => {
        this.form.data = data.auditoria
      }).catch(error => {
        console.error(error)
      })
      /* await axios.get('/reportes/getReporteAuditoria').then(({data}) => {
        this.form.data = data.auditoria
      }).catch(error => {
        console.error(error)
      }) */
    ...
    exportCSV () {
      let uri = process.env.API + `reportes/exportCSV_reporteAuditoria/${this.form.fields.estado_id}/${this.form.fields.municipio_id}/${this.form.fields.tipo}`
