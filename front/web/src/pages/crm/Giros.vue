          ...
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                ...
                <q-breadcrumbs-el class="q-ml-sm grey-8 fs28 page-title" label="Giros comerciales" />
              </q-breadcrumbs>
            </div>
          </div>
        ...
        <div class="row">
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                <q-breadcrumbs-el label="" icon="home" to="/dashboard" />
                <q-breadcrumbs-el label="Giros comerciales" to="" @click.native="setView('grid')"/>
                <q-breadcrumbs-el label="Nuevo giro"/>
              </q-breadcrumbs>
            </div>
          </div>
          <div class="col-sm-6 pull-right">
        ...
        <div class="row bg-white border-panel">
          <div class="col q-pa-md">
            <div class="row q-col-gutter-xs">
              <div class="col-sm-6">
                <q-field icon="grade" icon-color="dark" :error="$v.form.fields.nombre.$error" error-label="Ingrese el nombre del giro">
                  <q-input upper-case v-model="form.fields.nombre" type="text" inverted color="dark" float-label="Giro" maxlength="100" />
                </q-field>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      saveGiros: 'crm/giros/save',
      updateGiros: 'crm/giros/update',
      deleteGiros: 'crm/giros/delete'
    }),
    async loadAll () {
    ...
    save () {
      ...
      if (!this.$v.form.$error) {
        ...
        this.saveGiros(params).then(({data}) => {
    ...
    update () {
      ...
      if (!this.$v.form.$error) {
        this.loadingButton = true
        let params = this.form.fields
        this.updateGiros(params).then(({data}) => {
          this.loadingButton = false
          // this.$showMessage(data.message.title, data.message.content)
          if (data.result === 'success') {
            // ***
