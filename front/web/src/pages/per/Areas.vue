          ...
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                ...
                <q-breadcrumbs-el class="q-ml-sm grey-8 fs28 page-title" label="Áreas" />
              </q-breadcrumbs>
            </div>
          </div>
        ...
        <div class="row">
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                <q-breadcrumbs-el label="" icon="home" to="/dashboard" />
                <q-breadcrumbs-el label="Áreas" to="" @click.native="setView('grid')"/>
                <q-breadcrumbs-el label="Nueva área"/>
              </q-breadcrumbs>
            </div>
          </div>
          <div class="col-sm-6 pull-right">
        ...
        <div class="row bg-white border-panel">
          <div class="col q-pa-md">
            <div class="row q-col-gutter-xs">
              <div class="col-sm-6">
                <q-field icon="build" icon-color="dark" :error="$v.form.fields.nombre.$error" error-label="Ingrese el nombre del área">
...
export default {
  ...
  methods: {
    ...mapActions({
      ...
      saveAreas: 'per/areas/save',
      updateAreas: 'per/areas/update',
      deleteAreas: 'per/areas/delete'
    }),
    async loadAll () {
    ...
    save () {
      ...
      if (!this.$v.form.$error) {
        ...
        this.saveAreas(params).then(({data}) => {
    ...
    update () {
      ...
      if (!this.$v.form.$error) {
        this.loadingButton = true
        let params = this.form.fields
        this.updateAreas(params).then(({data}) => {
          this.loadingButton = false
          // this.$showMessage(data.message.title, data.message.content)
          if (data.result === 'success') {
            // ***
    ...
    deleteSingleRow (id) {
      this.$q.dialog({
        title: 'Confirmar',
        message: '¿Desea borrar esta área?',
        ok: 'Aceptar',
        cancel: 'Cancelar'
      }).then(() => {
        this.delete(id)
    ...
    delete (id) {
      let params = {id: id}
      this.deleteAreas(params).then(({data}) => {
        // this.$showMessage(data.message.title, data.message.content)
        if (data.result === 'success') {
          // ***
          this.$q.notify({
