          ...
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                ...
                <q-breadcrumbs-el class="q-ml-sm grey-8 fs28 page-title" label="Subconceptos" />
              </q-breadcrumbs>
            </div>
          </div>
          ...
          <div class="col q-pa-md" style="padding: 0;">
            ...
            <div class="row q-col-gutter-xs">
              <div class="col-sm-4">
                ...
                <q-field icon="fas fa-globe" icon-color="dark" :error="$v.form.fields.concepto_id.$error" error-label="Elija un concepto">
                  <q-select v-model="form.fields.concepto_id" inverted color="dark" float-label="Concepto" :options="conceptosOptions" filter @input="getData()"/>
                </q-field>
               </div>
              </div>
            </div>
        ...
        <div class="row">
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                <q-breadcrumbs-el label="" icon="home" to="/dashboard" />
                <q-breadcrumbs-el label="Subconceptos" to="" @click.native="setView('grid')"/>
                <q-breadcrumbs-el label="Nuevo subconcepto"/>
              </q-breadcrumbs>
            </div>
          </div>
          <div class="col-sm-6 pull-right">
        ...
        <div class="row bg-white border-panel">
          <div class="col q-pa-md">
            <div class="row q-col-gutter-xs">
              <div class="col-sm-6">
                <q-field icon="fas fa-globe" icon-color="dark" :error="$v.form.fields.concepto_id.$error" error-label="Elija un concepto">
                  <q-select v-model="form.fields.concepto_id" inverted color="dark" float-label="Concepto" :options="conceptosOptions" filter/>
                </q-field>
              </div>
              <div class="col-sm-6">
                <q-field icon="fas fa-globe" icon-color="dark" :error="$v.form.fields.nombre.$error" error-label="Ingrese el concepto">
                  <q-input upper-case v-model="form.fields.nombre" type="text" inverted color="dark" float-label="Nombre del concepto" maxlength="100" />
                </q-field>
              </div>
            </div>
          ...
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                <q-breadcrumbs-el label="" icon="home" to="/dashboard" />
                <q-breadcrumbs-el label="Subconceptos" to="" @click.native="setView('grid')"/>
                <q-breadcrumbs-el :label="form.fields.concepto_id"/>
              </q-breadcrumbs>
            </div>
          </div>
          <div class="col-sm-6 pull-right">
...
export default {
  ...
  beforeRouteEnter (to, from, next) {
    next(vm => {
      ...
      for (let i = 0; i < propiedades.length; i++) {
        if (propiedades[i].toUpperCase() === 'admin'.toUpperCase() || propiedades[i].toUpperCase() === 'ROOT'.toUpperCase() || propiedades[i].toUpperCase() === 'CLIENTE'.toUpperCase() || propiedades[i].toUpperCase() === 'LIDER'.toUpperCase() || propiedades[i].toUpperCase() === 'COORDINADOR'.toUpperCase() || propiedades[i].toUpperCase() === 'GERENTE DE OPERACIONES'.toUpperCase() || propiedades[i].toUpperCase() === 'PMO'.toUpperCase() || propiedades[i].toUpperCase() === 'FINANZAS'.toUpperCase() || propiedades[i].toUpperCase() === 'OPERACIÓN'.toUpperCase() || propiedades[i].toUpperCase() === 'LICITACIONES'.toUpperCase() || propiedades[i].toUpperCase() === 'GERENTE'.toUpperCase() || propiedades[i].toUpperCase() === 'COBRANZA'.toUpperCase() || propiedades[i].toUpperCase() === 'FINANZAS/COMISIONES'.toUpperCase() || propiedades[i].toUpperCase() === 'AUXILIAR TESORERIA'.toUpperCase()) {
          condicion = true
        }
      }
      if (condicion) {
  ...
  computed: {
    ...mapGetters({
      ...
      subconceptos: 'vnt/subconcepto/subconceptos'
    }),
    conceptosOptions () {
      let concepto = this.$store.getters['vnt/concepto/selectOptions']
      concepto.splice(0, 0, {label: '---Seleccione---', value: 0})
      return concepto
    }
  },
  created () {
  ...
  methods: {
    ...mapActions({
      ...
      getSubconceptos: 'vnt/subconcepto/refresh',
      saveSubconcepto: 'vnt/subconcepto/save',
      updateSubconcepto: 'vnt/subconcepto/update',
      deleteSubconcepto: 'vnt/subconcepto/delete'
    }),
    async loadAll () {
    ...
    getData () {
      this.form.data = []
      this.getSubconceptos({id: this.form.fields.concepto_id}).then(({data}) => {
        this.form.data = data.subconceptos
      }).catch(error => {
        console.error(error)
      })
    },
    ...
    save () {
      ...
      if (!this.$v.form.$error) {
        ...
        this.saveSubconcepto(params).then(({data}) => {
    ...
    update () {
      ...
      if (!this.$v.form.$error) {
        this.loadingButton = true
        let params = this.form.fields
        this.updateSubconcepto(params).then(({data}) => {
          this.loadingButton = false
          // this.$showMessage(data.message.title, data.message.content)
          if (data.result === 'success') {
            // ***
    ...
    editRow (row) {
      let subconcepto = { ...row }
      this.form.fields.id = subconcepto.id
      this.form.fields.nombre = subconcepto.nombre
      this.form.fields.concepto_id = subconcepto.concepto_id
      this.setView('update')
    },
    deleteSingleRow (id) {
      this.$q.dialog({
        title: 'Confirmar',
        message: '¿Desea borrar este subconcepto?',
        ok: 'Aceptar',
        cancel: 'Cancelar'
      }).then(() => {
        this.delete(id)
    ...
    delete (id) {
      let params = {id: id}
      this.deleteSubconcepto(params).then(({data}) => {
        // this.$showMessage(data.message.title, data.message.content)
        if (data.result === 'success') {
          // ***
          this.$q.notify({
  ...
  validations: {
    form: {
      fields: {
        ...
        concepto_id: {required, minValue: minValue(1)}
      }
    }
  }
}
