          ...
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                ...
                <q-breadcrumbs-el class="q-ml-sm grey-8 fs28 page-title" label="Conceptos" />
              </q-breadcrumbs>
            </div>
          </div>
        ...
        <div class="row">
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                <q-breadcrumbs-el label="" icon="home" to="/dashboard" />
                <q-breadcrumbs-el label="Conceptos" to="" @click.native="setView('grid')"/>
                <q-breadcrumbs-el label="Nuevo concepto"/>
              </q-breadcrumbs>
            </div>
          </div>
          <div class="col-sm-6 pull-right">
...
export default {
  ...
  computed: {
    ...mapGetters({
      conceptos: 'vnt/concepto/conceptos'
    })
  },
  created () {
    this.loadAll()
  ...
  methods: {
    ...mapActions({
      ...
      saveConcepto: 'vnt/concepto/save',
      updateConcepto: 'vnt/concepto/update',
      deleteConcepto: 'vnt/concepto/delete'
    }),
    async loadAll () {
    ...
    save () {
      ...
      if (!this.$v.form.$error) {
        ...
        this.saveConcepto(params).then(({data}) => {
    ...
    editRow (row) {
      let concepto = { ...row }
      this.form.fields.id = concepto.id
      this.form.fields.nombre = concepto.nombre
      this.setView('update')
    },
    deleteSingleRow (id) {
      this.$q.dialog({
        title: 'Confirmar',
        message: '¿Desea borrar este concepto?',
        ok: 'Aceptar',
        cancel: 'Cancelar'
      }).then(() => {
        this.delete(id)
    ...
    delete (id) {
      let params = {id: id}
      this.deleteConcepto(params).then(({data}) => {
        // this.$showMessage(data.message.title, data.message.content)
        if (data.result === 'success') {
          // ***
          this.$q.notify({
