          ...
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                ...
                <q-breadcrumbs-el class="q-ml-sm grey-8 fs28 page-title" label="Municipios"/>
              </q-breadcrumbs>
            </div>
          </div>
        ...
        <div class="row">
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                <q-breadcrumbs-el label="" icon="home" to="/dashboard" />
                <q-breadcrumbs-el label="Municipios" to="" @click.native="setView('grid')"/>
            ...
            <div class="row q-col-gutter-xs">
              <div class="col-sm-6">
                <q-field icon="fas fa-globe" icon-color="dark" :error="$v.form.fields.estado_id.$error" error-label="Elija un estado">
                  <q-select v-model="form.fields.estado_id" inverted color="dark" float-label="Estado" :options="estadosOptions" filter/>
                </q-field>
              </div>
              <div class="col-sm-6">
                <q-field icon="fas fa-map-pin" icon-color="dark" :error="$v.form.fields.nombre.$error" error-label="Ingrese un municipio">
...
export default {
  ...
  data () {
    return {
      ...
      form: {
        ...
        // data: [],
        columns: [
          { name: 'nombre', label: 'Municipio', field: 'nombre', sortable: true, type: 'string', align: 'left' },
  ...
  computed: {
    ...mapGetters({
      ...
      municipios: 'vnt/municipio/municipios'
    })
  },
  created () {
    this.loadAll()
  ...
  methods: {
    ...mapActions({
      getMunicipios: 'vnt/municipio/refresh',
      saveMunicipio: 'vnt/municipio/save',
      updateMunicipio: 'vnt/municipio/update',
      deleteMunicipio: 'vnt/municipio/delete',
    ...
    save () {
      ...
      if (!this.$v.form.$error) {
        ...
        this.saveMunicipio(params).then(({data}) => {
    ...
    update () {
      ...
      if (!this.$v.form.$error) {
        this.loadingButton = true
        let params = this.form.fields
        this.updateMunicipio(params).then(({data}) => {
          this.loadingButton = false
          if (data.result === 'success') {
            this.$q.notify({
              // only required parameter is the message:
    ...
    editRow (row) {
      ...
      this.form.fields.nombre = municipio.nombre
      this.form.fields.estado_id = municipio.estado_id
      this.setView('update')
    },
    deleteSingleRow (id) {
      this.$q.dialog({
    ...
    delete (id) {
      let params = {id: id}
      this.deleteMunicipio(params).then(({data}) => {
        if (data.result === 'success') {
          this.$q.notify({
            // only required parameter is the message:
            message: data.message.content,
