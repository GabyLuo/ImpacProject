          ...
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                ...
                <q-breadcrumbs-el class="q-ml-sm grey-8 fs28 page-title" label="Categorías" />
              </q-breadcrumbs>
            </div>
          </div>
        ...
        <div class="row">
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                <q-breadcrumbs-el label="" icon="home" to="/dashboard" />
                <q-breadcrumbs-el label="Categorías" to="" @click.native="setView('grid')"/>
        ...
        <div class="row bg-white border-panel">
          <div class="col q-pa-md">
            <div class="row q-col-gutter-xs">
              ...
              <div class="col-sm-6">
                <q-field icon="storage" icon-color="dark" :error="$v.form.fields.nombre.$error" error-label="Ingrese un nombre">
...
export default {
  ...
  computed: {
    ...mapGetters({
      tipos_articulos: 'inv/tipos_articulos/tipos_articulos'
    })
  },
  created () {
    this.loadAll()
  ...
  methods: {
    ...mapActions({
      ...
      saveTiposArticulos: 'inv/tipos_articulos/save',
      updateTiposArticulos: 'inv/tipos_articulos/update',
      deleteTiposArticulos: 'inv/tipos_articulos/delete'
    }),
    async loadAll () {
    ...
    save () {
      ...
      if (!this.$v.form.$error) {
        ...
        this.saveTiposArticulos(params).then(({data}) => {
    ...
    update () {
      ...
      if (!this.$v.form.$error) {
        this.loadingButton = true
        let params = this.form.fields
        this.updateTiposArticulos(params).then(({data}) => {
          this.loadingButton = false
          if (data.result === 'success') {
            this.$q.notify({
              // only required parameter is the message:
    ...
    deleteSingleRow (id) {
      this.$q.dialog({
        title: 'Confirmar',
        message: '¿Desea borrar esta categoría?',
        ok: 'Aceptar',
        cancel: 'Cancelar'
      }).then(() => {
        this.delete(id)
    ...
    delete (id) {
      let params = {id: id}
      this.deleteTiposArticulos(params).then(({data}) => {
        if (data.result === 'success') {
          this.$q.notify({
            // only required parameter is the message:
            message: data.message.content,
