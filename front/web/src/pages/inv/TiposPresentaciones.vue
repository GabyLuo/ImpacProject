          ...
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                ...
                <q-breadcrumbs-el class="q-ml-sm grey-8 fs28 page-title" label="Unidades" />
              </q-breadcrumbs>
            </div>
          </div>
                ...
                <q-table id="sticky-table-newstyle"
                  ...
                  <template slot="body" slot-scope="props">
                    <q-tr :props="props">
                      ...
                      <q-td key="codigo_sat" :props="props">{{ props.row.codigo_sat }}</q-td>
        ...
        <div class="row">
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                <q-breadcrumbs-el label="" icon="home" to="/dashboard" />
                <q-breadcrumbs-el label="Unidades" to="" @click.native="setView('grid')"/>
        ...
        <div class="row bg-white border-panel">
          <div class="col q-pa-md">
            <div class="row q-col-gutter-xs">
              ...
              <div class="col-sm-6">
                <q-field icon="widgets" icon-color="dark" :error="$v.form.fields.nombre.$error" error-label="Ingrese un nombre">
                  <q-input upper-case v-model="form.fields.nombre" type="text" inverted color="dark" float-label="Nombre" maxlength="100" />
                </q-field>
              </div>
              <div class="col-sm-2">
                <q-field icon="dialpad" icon-color="dark" :error="$v.form.fields.codigo_sat.$error" error-label="Ingrese un código del SAT">
                  <q-input upper-case v-model="form.fields.codigo_sat" type="text" inverted color="dark" float-label="Código del SAT" maxlength="3" />
                </q-field>
              </div>
            </div>
...
export default {
    ...
    return {
      ...
      form: {
        ...
        columns: [
          ...
          { name: 'codigo_sat', label: 'Código del SAT', field: 'codigo_sat', sortable: true, type: 'string', align: 'left' },
  ...
  computed: {
    ...mapGetters({
      tipos_presentaciones: 'inv/tipos_presentaciones/tipos_presentaciones'
    })
  },
  created () {
    this.loadAll()
  ...
  methods: {
    ...mapActions({
      getTiposPresentaciones: 'inv/tipos_presentaciones/refresh',
      saveTiposPresentaciones: 'inv/tipos_presentaciones/save',
      updateTiposPresentaciones: 'inv/tipos_presentaciones/update',
      deleteTiposPresentaciones: 'inv/tipos_presentaciones/delete'
    }),
    async loadAll () {
      this.form.loading = true
      await this.getTiposPresentaciones()
    ...
    save () {
      ...
      if (!this.$v.form.$error) {
        ...
        this.saveTiposPresentaciones(params).then(({data}) => {
    ...
    update () {
      ...
      if (!this.$v.form.$error) {
        this.loadingButton = true
        let params = this.form.fields
        this.updateTiposPresentaciones(params).then(({data}) => {
          this.loadingButton = false
          if (data.result === 'success') {
            this.$q.notify({
              // only required parameter is the message:
    ...
    editRow (row) {
      ...
      this.form.fields.codigo_sat = row.codigo_sat
      this.setView('update')
    },
    deleteSingleRow (id) {
      this.$q.dialog({
        title: 'Confirmar',
        message: '¿Desea borrar esta unidad?',
        ok: 'Aceptar',
        cancel: 'Cancelar'
      }).then(() => {
        this.delete(id)
    ...
    delete (id) {
      let params = {id: id}
      this.deleteTiposPresentaciones(params).then(({data}) => {
        if (data.result === 'success') {
          this.$q.notify({
            // only required parameter is the message:
            message: data.message.content,
