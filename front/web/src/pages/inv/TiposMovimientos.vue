          ...
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                ...
                <q-breadcrumbs-el class="q-ml-sm grey-8 fs28 page-title" label="Tipos de movimientos" />
              </q-breadcrumbs>
            </div>
          </div>
        ...
        <div class="row">
          <div class="col-sm-6">
            <div class="q-pa-sm q-gutter-sm">
              <q-breadcrumbs align="left">
                <q-breadcrumbs-el label="" icon="home" to="/dashboard" />
                <q-breadcrumbs-el label="Tipos de movimientos" to="" @click.native="setView('grid')"/>
                <q-breadcrumbs-el label="Nuevo tipo de movimiento"/>
              </q-breadcrumbs>
            </div>
          </div>
          <div class="col-sm-6 pull-right">
        ...
        <div class="row bg-white border-panel">
          <div class="col q-pa-md">
            <div class="row q-col-gutter-xs">
              <div class="col-sm-6">
                <q-field icon="swap_horiz" icon-color="dark" :error="$v.form.fields.nombre.$error" error-label="Ingrese un nombre">
...
export default {
  ...
  computed: {
    ...mapGetters({
      tipos_movimientos: 'inv/tipos_movimientos/tipos_movimientos'
    })
  },
  created () {
    this.loadAll()
  ...
  methods: {
    ...mapActions({
      ...
      saveTiposMovimientos: 'inv/tipos_movimientos/save',
      updateTiposMovimientos: 'inv/tipos_movimientos/update',
      deleteTiposMovimientos: 'inv/tipos_movimientos/delete'
    }),
    async loadAll () {
    ...
    save () {
      ...
      if (!this.$v.form.$error) {
        ...
        this.saveTiposMovimientos(params).then(({data}) => {
    ...
    update () {
      ...
      if (!this.$v.form.$error) {
        this.loadingButton = true
        let params = this.form.fields
        this.updateTiposMovimientos(params).then(({data}) => {
          this.loadingButton = false
          if (data.result === 'success') {
            this.$q.notify({
              // only required parameter is the message:
    ...
    delete (id) {
      let params = {id: id}
      this.deleteTiposMovimientos(params).then(({data}) => {
        if (data.result === 'success') {
          this.$q.notify({
            // only required parameter is the message:
            message: data.message.content,
