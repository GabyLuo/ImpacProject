    ...
    <q-slide-transition>
      <div v-show="showMe" class="bg-grey-4" style="border-bottom: inset; border-width: 0.5px; border-bottom-color: #e1e1e1;">
        <template v-for="item in sectionData.items" >
          <q-item :key="item.label" class="q-item-extend" item :to="item.route" @click.native="navegar(item)" v-if="item.route!=='/licitaciones' && item.route!=='/reporte_projects'">
            <q-item-main @click="navegar(item)">
              <q-icon :name="item.icon" />
              <label class="q-item-label">{{item.label}}</label>
            </q-item-main>
          </q-item> <!-- aqui cambiamos cosas -->
          <q-item :key="item.label" class="q-item-extend" item v-if="item.label==='Licitaciones' && item.route==='/licitaciones'">
            <q-item-main>
              <!-- aqui modificamos -->
          ...
          <q-item :key="item.label" class="q-item-extend" item v-if="item.label==='Auditoría' && item.route==='/reporte_projects'">
            <q-item-main>
              <!-- aqui modificamos -->
              <div class="q-list-header-helper" @click="showMe3 = !showMe3" v-if="item.label==='Auditoría'">
                <div class="row">
                  <div class="col-xs-7">
                    <q-icon :name="item.icon" />
                    <label class="q-item-label">{{item.label}}</label>
                  ...
                  <div class="col-xs-1 offset-xs-4">
                    <i class="iconoCatalogo" :class="[showMe3 ? '' : 'fa fa-angle-right', 'fa fa-angle-down']"></i>
                  </div>
                </div>
              </div>
              <div v-show="showMe3" class="bg-grey-4" v-if="item.label==='Auditoría'" style="margin-top: 15px;">
                <template>
                  <q-item key="Licitaciones" class="q-item-extend" item to="/reporte_projects" @click.native="navegarSingle('/licitaciones')">
                    <q-item-main @click="navegarSingle('/reporte_projects')">
                      <q-icon name="local_library" />
                      <label class="q-item-label">Contratos</label>
                    </q-item-main>
                  </q-item>
                  <q-item key="Fianzas" class="q-item-extend" item to="/reporte_projects2" @click.native="navegarSingle('/fianzas')">
                    <q-item-main @click="navegarSingle('/reporte_projects2')">
                      <q-icon name="folder" />
                      <label class="q-item-label">Presupuestos</label>
                    </q-item-main>
                  </q-item>
                  <!-- <q-item key="Repositorio" class="q-item-extend" item to="/repositorio" @click.native="navegarSingle('/repositorio')">
                    <q-item-main @click="navegarSingle('/repositorio')">
